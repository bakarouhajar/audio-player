...
class Visualisation3d extends HTMLElement {
    ...
    async connectedCallback() {
        ...
        document.addEventListener('analyserReady', (event) => {
            this.audioPlayer = event.target; // my-audio-player
            if (!this.audioPlayer) {
        ...
        document.addEventListener('analyserReady', (event) => {
            this.analyser = event.detail.analyser;
            console.log("Analyser received in Visualisation3D:", this.analyser);
        });
    ...
    create3DVisualization() {
        ...
        camera.useAutoRotationBehavior = true; // Activer la rotation automatique si nécessaire
        camera.lowerRadiusLimit = 10; // Limite minimale de zoom
        camera.upperRadiusLimit = 50; // Limite maximale de zoom
